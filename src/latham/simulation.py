...
class State:
    def __init__(
        ...
        self.connectivity: csr_array[float32] = State._get_connection_matrix(
            N_I,
            self.positions,
            cell_params,
            synaptic_params,
    ...
    def _get_connection_matrix(
        ...
        connections = csr_array(rng.random((network_params.N, network_params.N)) < P)
        ...
        return (connections * weights).tocsr()
    ...
    def process_spikes(self, cell_params, synaptic_params):
        ...
        self.synaptic_currents += sp.sparse.vstack(
            [
                csr_array(self.connectivity @ spikes),
                csr_array(self.connectivity @ (spikes * self.rev_potentials)),
            ],
            format="csr",
            dtype=float32,
        )
